 ...
 *   2. `BUILD_TIME` / `GIT_COMMIT` / `GIT_BRANCH` env overrides.
 *   3. `package.json` version + its mtime as a coarse build-time fallback.
 * Always includes `startedAt` (module load time). Never throws.
 */
function getBuildInfo() {
  ...
  const info = {
    ...
    branch: stamp.branch || process.env.GIT_BRANCH || null,
